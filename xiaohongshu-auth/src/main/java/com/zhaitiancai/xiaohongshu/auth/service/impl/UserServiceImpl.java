...
import com.zhaitiancai.framework.common.eumns.DeletedEnum;
import com.zhaitiancai.framework.common.eumns.StatusEnum;
...
import com.zhaitiancai.xiaohongshu.auth.constant.RoleConstants;
import com.zhaitiancai.xiaohongshu.auth.domain.dataobject.UserDO;
import com.zhaitiancai.xiaohongshu.auth.domain.dataobject.UserRoleDO;
import com.zhaitiancai.xiaohongshu.auth.enums.LoginTypeEnum;
...
import com.zhaitiancai.xiaohongshu.auth.mapper.UserRoleDOMapper;
...
public class UserServiceImpl implements UserService {
	...
	public Response<String> loginAndRegister(UserLoginReqVO userLoginReqVO) {
		...
		final Integer type = userLoginReqVO.getType();
		final LoginTypeEnum loginTypeEnum = LoginTypeEnum.valueOf(type);
		Long userId=null;
		//判断登录类型
		switch (loginTypeEnum){
			case VERIFICATION_CODE://验证码登录
				...
				//校验入参验证码是否为空
				Preconditions.checkArgument(StringUtils.isNotBlank(verificationCode));
				//构建验证码 Redis Key
				final String key = RedisKeyConstants.buildVerificationCodeKey(phone);
				//查询储存在 Redis 中该用户的登录验证码
				final String sentCode = (String) redisTemplate.opsForValue().get(key);
